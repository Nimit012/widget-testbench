...
<template>
  <div>
    <div class="flex items-center gap-2 mb-3">
      <span
        class="w-[34px] h-[34px] rounded-[9px] bg-[var(--w-color-kicker-icon-bg,#e6edfc)] text-[var(--w-color-kicker,#2f65c3)] inline-flex items-center justify-center"
      >
        <img
      ...
      <span
        class="m-0 text-[clamp(0.95rem,1.02vw,1rem)] font-bold tracking-[0.01em] text-[#2f65c3]"
        >Question {{ questionIndex }}</span
      >
    </div>
...
onMounted(async () => {
  ...
  if (!activityJson || !launchId) {
    console.error(
      "[ActivityBlock] activityJson or block.id is missing — assessment will not be initialised.",
      {
        activityJson,
        launchId,
      },
  ...
  const unknownTypes = uniqueQuestionTypes.filter(
    (t) => !QUESTION_PACKAGE_MAP[t],
  );
  if (unknownTypes.length) {
    console.warn(
      `[ActivityBlock] No package mapped for question type(s): ${unknownTypes.join(", ")}`,
    );
  }

  // ── Import everything in parallel (singleton cache prevents duplicate fetches) ───
  const [activityModule] = await Promise.all([
    ensureImported(subType, ACTIVITY_PACKAGE_MAP[subType]).then(() =>
      ACTIVITY_PACKAGE_MAP[subType](),
    ),
    ...uniqueQuestionTypes
  ...
  const AssessmentClass =
    (exportName && activityModule[exportName]) ??
    Object.values(activityModule)[0];
    ...
    events: {
      ...
      submit: (e: any) =>
        emit("on-user-response", {
          ...
          responses: [{ id: props.block.id, value: JSON.stringify(e) }],
        }),
