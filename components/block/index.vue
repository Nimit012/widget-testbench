...
<template>
  <div class="block-wrapper mb-6 font-[Satoshi,sans-serif]">
    ...
    <!-- v-show keeps the element in the DOM so the web-component can
         initialise and receive props; it is just hidden until 'ready'. -->
    <component
      v-if="resolvedComponent"
      v-show="isReady"
      :is="resolvedComponent"
      v-bind="resolvedProps"
      @ready="_onWidgetReady"
...
const resolvedComponent = computed(
  () => BLOCK_TYPE_TO_COMPONENT[props.block.type] ?? null,
);

/**
 * Tracks whether the block has fired its 'ready' event.
 * false = block hidden, true = block shown.
 */
...
const resolvedProps = computed((): Record<string, any> => {
  switch (props.block.type) {
    ...
    case "activity":
      return {
        block: props.block,
        segmentStatus: props.segmentStatus,
...
/**
 * Called when the block component fires its 'ready' event.
 * Flips isReady so the block becomes visible and emits on-block-ready upward.
 */
function _onWidgetReady() {
  isReady.value = true;
  emit('on-block-ready', { blockId: props.block.id });
}
...
<style scoped>
/**
 * Spinner overlay — shown while w-rt-block is initialising.
 * Sits above the block content area with a minimum height so the
 * layout does not collapse to zero before the widget renders.
 */
.rt-loader {
  display: flex;
  align-items: center;
