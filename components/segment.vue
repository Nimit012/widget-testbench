...
<template>
  ...
  >
    ...
    >
      <!-- CONTENT AREA -->
      <div class="lg:px-[176px] px-6 py-10 relative">
        <!-- Segment-level loader overlay -->
        <div v-if="isSegmentLoading" class="segment-loader-overlay">
          <span class="segment-spinner" aria-label="Loading segment..." />
        </div>
        <template v-if="segmentContent?.blocks?.length > 0">
          <Block
            ...
            @on-block-ready="_handleBlockReady"
            v-show="!isSegmentLoading"
          />
        </template>
      ...
      <!-- ACTION BUTTONS -->
      <div
        class="flex justify-start gap-3 px-[clamp(80px,7.83vw,125px)] py-8 bg-white border-t border-[#A2B2BE]"
      >
...
// Segment-level loader state
const isSegmentLoading = shallowRef(true);
const blocksReadyCount = shallowRef(0);
const totalBlocks = computed(() => props.segmentContent?.blocks?.length || 0);

function _handleBlockReady() {
  blocksReadyCount.value++;
  if (blocksReadyCount.value >= totalBlocks.value) {
    isSegmentLoading.value = false;
  }
}

// Reset loader state on mount/expand
import { onMounted, watch } from 'vue';
onMounted(() => {
  isSegmentLoading.value = true;
  blocksReadyCount.value = 0;
});
watch(
  () => isExpanded.value,
  (expanded) => {
